...
// Groups findings by (technician, work week) — one group is one disciplinary
// conversation. techShiftMap: Map<technicianName, shiftCode>.
export function groupFindingsByWorkWeek(findings, techShiftMap) {
...
// The "needs action" count that matters to a leader — one bundle (one
// conversation), not one per raw finding.
export function countPendingActions(findings, techShiftMap) {
  return groupFindingsByWorkWeek(findings, techShiftMap).length
}
